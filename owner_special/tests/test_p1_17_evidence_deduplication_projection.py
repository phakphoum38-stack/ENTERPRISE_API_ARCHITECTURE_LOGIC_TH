...
from owner_special.research_os_friend.p1_17_evidence_deduplication_projection import project_p1_17, P117Error


def test_p1_17_projection():
    i = CanonicalIdentity(mission_id="m", work_id="w", baseline_sha="a"*40)
    b = ExecutionEvidenceBinding(
        identity=i, resource_binding_hash="1"*64, evidence_id="EV-17",
    ...
    assert p.evidence_ids == ("EV-17",)
    assert len(p.projection_hash) == 64


def test_p1_17_fails_closed_on_lineage():
    i = CanonicalIdentity(mission_id="m", work_id="w", baseline_sha="a"*40)
    ...
    try:
        project_p1_17(identity=i, bindings=[b], fingerprint="4"*64)
    except P117Error:
        return
