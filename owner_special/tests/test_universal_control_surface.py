...
from tools.universal_control_surface import (
    CommandSpec,
    SurfaceObject,
...
class UniversalControlSurfaceTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.surface.register_object(
            SurfaceObject(
                object_id="TASK-001",
                object_type="TASK",
                title="Research control center",
                state="OBSERVED",
                relations=("EV-001", "SERVICE-001"),
        ...
        self.surface.register_object(
            SurfaceObject(
                ...
                title="Control Center runtime evidence",
                state="VERIFIED",
        ...
        self.surface.register_object(
            SurfaceObject(
                object_id="SERVICE-001",
                object_type="SERVICE",
                title="Owner Friend runtime",
                state="RUNNING",
            )
        )
    ...
    def test_command_preparation_is_not_execution(self) -> None:
        self.surface.register_command(
            CommandSpec(
                command_id="CMD-001",
                label="Inspect runtime",
                intent="Observe runtime state",
                target="SERVICE-001",
                mode="SIMULATION",
            )
        )
