...
def test_p1_22_projection():
    ...
    b=ExecutionEvidenceBinding(i,"1"*64,"EV-22","2"*64,"VERIFIED",("ref",),"3"*64)
    p=project_p1_22(identity=i,bindings=[b],fingerprint="4"*64)
    assert p.evidence_ids==("EV-22",) and len(p.projection_hash)==64
def test_p1_22_fails_closed():
    i=CanonicalIdentity("m","w","a"*40); o=CanonicalIdentity("x","w","a"*40)
    b=ExecutionEvidenceBinding(o,"1"*64,"EV-X","2"*64,"VERIFIED",("ref",),"3"*64)
    try: project_p1_22(identity=i,bindings=[b],fingerprint="4"*64)
    except P122Error: return
