...
def _result(*, status: str, detail: str = "known_failure") -> ControlResult:
    return ControlResult(
        ...
        detail=detail,
        evidence_id=hashlib.sha256(f"{CONTROL_ID}|{1 if status != 'PASS' else 0}|{detail}".encode("utf-8")).hexdigest(),
        failure_signature=(
            ...
            "controls": [
                {
                    ...
                    "evidence_id": hashlib.sha256(f"{CONTROL_ID}|1|known_failure".encode("utf-8")).hexdigest(),
