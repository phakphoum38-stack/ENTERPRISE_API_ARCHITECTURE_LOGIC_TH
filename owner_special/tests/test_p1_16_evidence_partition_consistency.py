...
from owner_special.research_os_friend.p1_16_evidence_partition_consistency import project_p1_16, P116Error


def test_p1_16_projection():
    i = CanonicalIdentity(mission_id="m", work_id="w", baseline_sha="a"*40)
    b = ExecutionEvidenceBinding(
        identity=i, resource_binding_hash="1"*64, evidence_id="EV-16",
        provenance_hash="2"*64, verification_status="VERIFIED",
        evidence_refs=("ref",), verification_fingerprint="3"*64
    )
    p = project_p1_16(identity=i, bindings=[b], fingerprint="4"*64)
    assert p.evidence_ids == ("EV-16",)
    assert len(p.projection_hash) == 64


def test_p1_16_fails_closed_on_lineage():
    i = CanonicalIdentity(mission_id="m", work_id="w", baseline_sha="a"*40)
    ...
    try:
        project_p1_16(identity=i, bindings=[b], fingerprint="4"*64)
    except P116Error:
        return
