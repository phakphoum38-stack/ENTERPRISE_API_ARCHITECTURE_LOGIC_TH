...
from owner_special.research_os_friend.p1_18_evidence_ordering import project_p1_18, P118Error
def test_p1_18_projection():
    i=CanonicalIdentity("m","w","a"*40)
    b=ExecutionEvidenceBinding(i,"1"*64,"EV-18","2"*64,"VERIFIED",("ref",),"3"*64)
    p=project_p1_18(identity=i,bindings=[b],fingerprint="4"*64)
    assert p.evidence_ids==("EV-18",) and len(p.projection_hash)==64
def test_p1_18_fails_closed():
    i=CanonicalIdentity("m","w","a"*40); o=CanonicalIdentity("x","w","a"*40)
    b=ExecutionEvidenceBinding(o,"1"*64,"EV-X","2"*64,"VERIFIED",("ref",),"3"*64)
    try: project_p1_18(identity=i,bindings=[b],fingerprint="4"*64)
    except P118Error: return
