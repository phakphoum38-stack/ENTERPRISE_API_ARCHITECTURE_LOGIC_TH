...
class MissionControlUIProjectionTests(unittest.TestCase):
    ...
    def test_all_existing_panel_types_are_preserved(self) -> None:
        result = MissionControlUIProjection().project(snapshot(), owner_id=OWNER)
        self.assertEqual({panel["type"] for panel in result["panels"]}, {"text", "metric", "status", "table", "timeline", "capability-health"})
    ...
    def test_snapshot_owner_mismatch_is_rejected(self) -> None:
        bad = snapshot(); bad["owner_id"] = "other-owner"
        with self.assertRaises(MissionControlUIProjectionError): MissionControlUIProjection().project(bad, owner_id=OWNER)

    def test_read_only_is_required(self) -> None:
        bad = snapshot(); bad["read_only"] = False
        with self.assertRaises(MissionControlUIProjectionError): MissionControlUIProjection().project(bad, owner_id=OWNER)

    def test_conflicting_snapshot_schema_is_rejected(self) -> None:
        bad = snapshot(); bad["schema"] = "research-os-mission-control-unified-snapshot/v999"
        with self.assertRaises(MissionControlUIProjectionError): MissionControlUIProjection().project(bad, owner_id=OWNER)

    def test_malformed_ui_panel_is_rejected(self) -> None:
        bad = snapshot(); bad["ui_schema"]["panels"] = [{"id": "x", "type": "execute"}]  # type: ignore[index]
        with self.assertRaises(MissionControlUIProjectionError): MissionControlUIProjection().project(bad, owner_id=OWNER)

    def test_secret_like_content_is_rejected(self) -> None:
        bad = snapshot(); bad["ui_schema"]["panels"][0]["title"] = "Bearer abc123"  # type: ignore[index]
        with self.assertRaises(MissionControlUIProjectionError): MissionControlUIProjection().project(bad, owner_id=OWNER)

    def test_executable_content_is_rejected(self) -> None:
        bad = snapshot(); bad["trace"]["command"] = "powershell -Command whoami"  # type: ignore[index]
        with self.assertRaises(MissionControlUIProjectionError): MissionControlUIProjection().project(bad, owner_id=OWNER)

    def test_dynamic_content_is_rejected(self) -> None:
        bad = snapshot(); bad["trace"]["dynamic"] = object()  # type: ignore[index]
        with self.assertRaises(MissionControlUIProjectionError): MissionControlUIProjection().project(bad, owner_id=OWNER)

    def test_panel_bound_is_enforced(self) -> None:
        bad = snapshot(); bad["ui_schema"]["panels"] = [{"id": f"panel-{index:02d}", "type": "text", "title": "x", "value": "x"} for index in range(33)]  # type: ignore[index]
        with self.assertRaises(MissionControlUIProjectionError): MissionControlUIProjection().project(bad, owner_id=OWNER)
    ...
    def test_statuses_are_not_reinterpreted(self) -> None:
        source = snapshot(); source["gate_status"]["overall_status"] = "FAILED"  # type: ignore[index]
    ...
    def test_input_is_not_mutated(self) -> None:
        source = snapshot(); original = copy.deepcopy(source)
        MissionControlUIProjection().project(source, owner_id=OWNER)
        self.assertEqual(source, original)

    def test_repeated_identical_input_is_identical(self) -> None:
        source = snapshot()
        self.assertEqual(MissionControlUIProjection().project(source, owner_id=OWNER), MissionControlUIProjection().project(source, owner_id=OWNER))

    def test_authority_replacement_is_rejected(self) -> None:
        bad = snapshot(); bad["execution_authority"] = "ReleaseController"
        with self.assertRaises(MissionControlUIProjectionError): MissionControlUIProjection().project(bad, owner_id=OWNER)
