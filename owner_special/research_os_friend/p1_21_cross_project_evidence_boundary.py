...
class P121Error(CanonicalIdentityError):
    """Raised when the P1-21 contract cannot be proven."""
...
def project_p1_21(
    *, identity: CanonicalIdentity,
    ...
    if not isinstance(fingerprint, str) or len(fingerprint) != 64:
        raise P121Error("fingerprint must be SHA-256")
    items = tuple(bindings)
    if not items:
        raise P121Error("at least one evidence binding is required")
    for item in items:
        if item.identity != identity:
            raise P121Error("evidence crosses canonical lineage")
        if not isinstance(item.binding_hash, str) or len(item.binding_hash) != 64:
            raise P121Error("binding hash must be SHA-256")
        if not isinstance(item.evidence_id, str) or not item.evidence_id.strip():
            raise P121Error("evidence_id is required")
    return P121Projection(
        identity=identity,
...
def assert_p1_21_lineage(
    projection: P121Projection, identity: CanonicalIdentity
) -> None:
    if projection.identity != identity:
        raise P121Error("projection lineage mismatch")
