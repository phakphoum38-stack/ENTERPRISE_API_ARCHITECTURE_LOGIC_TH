"""Core fail-closed AEOS assurance check fabric.

The fabric verifies the assurance universe and external assurance reports.
It never manufactures evidence, authority, certification, or PASS state.
"""
...
class AssuranceCheckError(ValueError):
    """Raised when assurance evidence is not admissible."""
...
def validate_registry(path: Path = REGISTRY) -> dict[str, Any]:
    data = _load(path)
    ...
    for item in checks:
        ...
        if boundary is not None and not (ROOT / boundary).is_file():
...
def validate_report(report: Mapping[str, Any], registry: Mapping[str, Any] | None = None, expected_sha: str | None = None) -> dict[str, Any]:
    ...
    for check_id in required:
        ...
        if status == "PASS":
            if type(entry.get("independent")) is not bool or entry["independent"] is not True:
                failures.append(f"{check_id}:PASS without independent=true")
            if type(entry.get("fresh")) is not bool or entry["fresh"] is not True:
...
def validate_exact_head(expected_sha: str) -> str:
    _validate_sha(expected_sha, "expected_sha")
    actual = _git_head()
