...
class P118Error(CanonicalIdentityError):
    """Raised when the P1-18 contract cannot be proven."""
...
def project_p1_18(
    *, identity: CanonicalIdentity,
    ...
    if not isinstance(fingerprint, str) or len(fingerprint) != 64:
        raise P118Error("fingerprint must be SHA-256")
    items = tuple(bindings)
    if not items:
        raise P118Error("at least one evidence binding is required")
    for item in items:
        if item.identity != identity:
            raise P118Error("evidence crosses canonical lineage")
        if not isinstance(item.binding_hash, str) or len(item.binding_hash) != 64:
            raise P118Error("binding hash must be SHA-256")
        if not isinstance(item.evidence_id, str) or not item.evidence_id.strip():
            raise P118Error("evidence_id is required")
    return P118Projection(
        identity=identity,
...
def assert_p1_18_lineage(
    projection: P118Projection, identity: CanonicalIdentity
) -> None:
    if projection.identity != identity:
        raise P118Error("projection lineage mismatch")
