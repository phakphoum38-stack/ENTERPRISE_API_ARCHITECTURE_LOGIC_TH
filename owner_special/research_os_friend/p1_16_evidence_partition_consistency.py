...
class P116Error(CanonicalIdentityError):
    """Raised when the P1-16 contract cannot be proven."""


@dataclass(frozen=True)
class P116Projection:
...
def project_p1_16(
    *, identity: CanonicalIdentity,
    ...
    if not isinstance(fingerprint, str) or len(fingerprint) != 64:
        raise P116Error("fingerprint must be SHA-256")
    items = tuple(bindings)
    if not items:
        raise P116Error("at least one evidence binding is required")
    for item in items:
        if item.identity != identity:
            raise P116Error("evidence crosses canonical lineage")
        if not isinstance(item.binding_hash, str) or len(item.binding_hash) != 64:
            raise P116Error("binding hash must be SHA-256")
        if not isinstance(item.evidence_id, str) or not item.evidence_id.strip():
            raise P116Error("evidence_id is required")
    return P116Projection(
        identity=identity,
...
def assert_p1_16_lineage(
    projection: P116Projection, identity: CanonicalIdentity
) -> None:
    if projection.identity != identity:
        raise P116Error("projection lineage mismatch")
