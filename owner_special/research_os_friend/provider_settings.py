...
class WindowsDpapiSecretStore:
    """Windows-native DPAPI machine-scoped encrypted secret storage."""
...
class ProviderManager:
    ...
    def provider(self) -> OpenAICompatibleProvider | None:
        ...
        if not config.enabled or not key or not config.base_url or not config.model:
            return None
    ...
    def test(self) -> dict[str, object]:
        ...
        if provider is None:
            return {**self.safe_status(), "connected": False, "error": "provider_not_ready"}
        try:
            return {**self.safe_status(), **provider.test_connection()}
        except Exception as exc:
            return {**self.safe_status(), "connected": False, "error": type(exc).__name__}
    ...
    def safe_status(self) -> dict[str, object]:
        ...
        return {
            ...
            "model": config.model,
            "credential_present": bool(self.secret_store.read()),
            "secret_backend": self.secret_store.backend,
