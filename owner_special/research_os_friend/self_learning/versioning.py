...
class SkillVersionProposal:
    """Immutable version proposal; it never mutates registered skills."""
...
def propose_next_version(
    ...
    *,
    ...
    """Calculate the next version while preserving explicit parent lineage."""
    parent_version = existing.version if existing is not None else None
    next_version = (
        parent_version + 1
        if parent_version is not None
        else max(1, candidate.version)
    )
