...
class _OwnerFriendAppState extends State<OwnerFriendApp> {
  int _index = 0;
  final OwnerNotificationCenter _notifications = OwnerNotificationCenter.instance;

  @override
  void initState() {
    super.initState();
    _notifications.startSession(
      serviceConnected: widget.startupError == null,
      startupError: widget.startupError,
    );
  }

  void _selectDestination(int value) {
    if (value == 4) _notifications.markAllRead();
    ...
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        ...
        return Badge(
          ...
          child: Icon(icon, key: const Key('notifications-nav-icon')),
        );
      },
    );
  }
  ...
  Widget build(BuildContext context) {
    final pages = <Widget>[
      _FriendChatPage(api: widget.api, notifications: _notifications),
      _CapabilitiesPage(api: widget.api, startup: widget.startup),
      _MemoryPage(api: widget.api),
      _ProviderPage(api: widget.api, notifications: _notifications),
      OwnerNotificationsPage(center: _notifications),
    ];
          ...
          actions: <Widget>[
            AnimatedBuilder(
              ...
              builder: (context, _) {
                final unread = _notifications.unreadCount;
                return IconButton(
                  key: const Key('notifications-appbar'),
            ...
            NavigationRail(
              ...
              destinations: <NavigationRailDestination>[
                ...
                NavigationRailDestination(
                  ...
                  selectedIcon: _notificationIcon(Icons.notifications),
...
class _FriendChatPage extends StatefulWidget {
  const _FriendChatPage({required this.api, required this.notifications});

  final OwnerFriendApi api;
  final OwnerNotificationCenter notifications;
...
class _FriendChatPageState extends State<_FriendChatPage> {
  ...
  Future<void> _send() async {
    ...
    try {
      ...
      final scale = decision['scale']?.toString() ?? '-';
      final capacity = (decision['capacity'] as num?)?.toInt() ?? 0;
      final activeWorkers = (helpers['active_workers'] as num?)?.toInt() ?? 0;
      final batches = (helpers['batches'] as num?)?.toInt() ?? 0;
      final factoryText = (factory['stages'] as List? ?? const <Object>[]).join(' → ');
      if (!mounted) return;
      setState(() {
        ...
        _batches = batches;
        _factory = factoryText;
      });
      widget.notifications.addSuccess(
        'Friend response ready',
        'Scale $scale • logical capacity $capacity • active workers $activeWorkers',
      );
    } catch (error) {
      if (!mounted) return;
      setState(() => _answer = 'Friend Service error: $error');
      widget.notifications.addError('Friend request failed', '$error');
    } finally {
...
class _ProviderPage extends StatefulWidget {
  const _ProviderPage({required this.api, required this.notifications});
...
class _ProviderPageState extends State<_ProviderPage> {
  ...
  Future<void> _load() async {
    try {
      ...
      widget.notifications.addError('Provider status unavailable', '$error');
    }
  }
  ...
  Future<void> _saveAndTest() async {
    ...
    try {
      ...
      setState(() {
        _status = saved;
        _message = connected ? 'Provider connected' : 'Provider test failed: ${tested['error'] ?? 'unknown'}';
      });
      if (connected) {
        widget.notifications.addSuccess(
          'Provider connected',
          'Model ${saved['model'] ?? '-'} is ready through the configured OpenAI-compatible endpoint.',
        );
      } else {
        widget.notifications.addWarning('Provider test failed', '${tested['error'] ?? 'unknown'}');
      }
    } catch (error) {
      if (!mounted) return;
      setState(() => _message = '$error');
      widget.notifications.addError('Provider configuration failed', '$error');
    } finally {
      if (mounted) setState(() => _busy = false);
    }
  }
