...
class _OwnerFriendAppV3State extends State<OwnerFriendAppV3> {
  ...
  Widget build(BuildContext context) {
    ...
    const sections = <String>['Chat', 'Capabilities', 'Memory', 'Provider'];
          ...
          child: Row(
            children: <Widget>[
              ...
              Expanded(
                child: Column(
                  children: <Widget>[
                    _TopBar(title: sections[_index], connected: connected),
                    ...
                    Expanded(
                      child: IndexedStack(
                        ...
                        children: <Widget>[
                          ...
                          _Capabilities(api: widget.api, startup: widget.startup),
...
enum _ChatMode { normal, files, library, image, web, deepResearch, calendar }
...
extension _ChatModeUi on _ChatMode {
  ...
  String get subtitle => switch (this) {
        ...
        _ChatMode.files => 'เลือกไฟล์จากเครื่องและส่งบริบทให้ Friend',
        _ChatMode.library => 'เลือกโฟลเดอร์คลังความรู้จากเครื่อง',
        _ChatMode.image => 'เตรียมคำสั่งสร้างภาพสำหรับ Provider',
        _ChatMode.web => 'เปิดผลค้นหาเว็บจริงในเบราว์เซอร์พร้อมส่งคำถามให้ Friend',
        _ChatMode.deepResearch => 'เพิ่มความซับซ้อน การวิเคราะห์ และการตรวจคุณภาพ',
        _ChatMode.calendar => 'เปิด Google Calendar และทำงานต่อกับ Friend',
      };
...
class _Attachment {
  const _Attachment({required this.path, required this.label, required this.context});
...
class _FriendChat extends StatefulWidget {
  const _FriendChat({required this.api});
...
class _FriendChatState extends State<_FriendChat> {
  ...
  static const Set<String> _textExtensions = <String>{
    'txt', 'md', 'json', 'yaml', 'yml', 'csv', 'log', 'py', 'dart', 'js', 'ts',
    'html', 'css', 'ps1', 'xml', 'ini', 'toml', 'sql', 'java', 'kt', 'swift', 'c',
    'cpp', 'h', 'hpp', 'cs', 'sh', 'bat',
  };
  ...
  Future<void> _handleMode(_ChatMode mode) async {
    ...
    switch (mode) {
      ...
      case _ChatMode.calendar:
        await _openExternal('https://calendar.google.com/');
        if (mounted) {
          setState(() => _toolStatus = 'เปิด Google Calendar แล้ว คุณสามารถพิมพ์งานที่ต้องการให้ Friend ช่วยต่อได้');
        }
      case _ChatMode.web:
        setState(() => _toolStatus = 'พิมพ์คำค้นแล้วกดส่ง ระบบจะเปิดผลค้นหาเว็บจริงพร้อมส่งคำถามให้ Friend');
      case _ChatMode.image:
        setState(() => _toolStatus = 'โหมดสร้างรูปภาพจะให้ Friend เตรียม image brief/prompt สำหรับ Provider ที่เชื่อมอยู่');
      case _ChatMode.deepResearch:
        setState(() => _toolStatus = 'Deep Research ใช้ complexity/parallelism สูงขึ้นและเปิด research + quality skills');
  ...
  Future<void> _pickFiles() async {
    ...
    setState(() {
      ...
      _toolStatus = added.isEmpty ? 'ไม่พบไฟล์ที่อ่านได้' : 'เพิ่มไฟล์ ${added.length} รายการแล้ว';
    });
  }
  ...
  Future<void> _pickLibraryFolder() async {
    ...
    try {
      await for (final entity in directory.list(recursive: true, followLinks: false)) {
        if (entity is File) {
          final relative = entity.path.startsWith(directory.path)
              ? entity.path.substring(directory.path.length).replaceFirst(RegExp(r'^[\\/]'), '')
              : entity.path;
          lines.add(relative);
    ...
    final context = <String>[
      ...
      'Files visible to the selected library context (${lines.length}${lines.length >= 120 ? '+' : ''}):',
      ...lines.map((item) => '- $item'),
    ].join('\n');

    setState(() {
      _attachments.add(_Attachment(path: path, label: 'Library: $label', context: context));
      _toolStatus = 'เพิ่มคลัง $label แล้ว';
    });
  }

  Future<_Attachment?> _attachmentFromFile(String path) async {
    ...
    if (_textExtensions.contains(extension) && size <= _maxTextFileBytes) {
      try {
        ...
        context.writeln('Content could not be decoded as UTF-8 text.');
      }
    } else {
      context.writeln('Binary or large file: metadata only in this runtime.');
    }

    return _Attachment(path: path, label: name, context: context.toString());
  }
  ...
  String _attachmentContext() {
    ...
    for (final attachment in _attachments) {
      if (output.length + attachment.context.length > _maxAttachmentContextChars) {
        output.writeln('\n[Attachment context truncated at $_maxAttachmentContextChars characters]');
        break;
      }
      output
        ..writeln('\n--- ${attachment.label} ---')
  ...
  Future<void> _send() async {
    ...
    if (visibleText.isEmpty || _busy) return;

    if (_mode == _ChatMode.web) {
      final searchUrl = 'https://www.google.com/search?q=${Uri.encodeQueryComponent(visibleText)}';
      await _openExternal(searchUrl);
    }

    var prompt = visibleText;
    if (_mode == _ChatMode.image) {
      prompt = 'Prepare a production-ready image generation brief and prompt for this request. '
          'Do not claim an image was generated unless the connected provider actually returns one.\n\n$visibleText';
    } else if (_mode == _ChatMode.web) {
      prompt = 'Web-search mode is active. The system opened a real browser search for this query. '
          'Do not claim to have read browser results unless the user supplies them. Analyze the query and explain what to verify.\n\n$visibleText';
    } else if (_mode == _ChatMode.calendar) {
      prompt = 'Google Calendar mode is active and the calendar was opened on the desktop. '
          'Help plan or interpret the requested schedule without claiming calendar data that was not supplied.\n\n$visibleText';
    }
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        _turns.add(_ChatTurn(user: visibleText, answer: answer, provider: provider));
        _provider = provider;
      ...
      if (!mounted) return;
      setState(() {
        _turns.add(_ChatTurn(user: visibleText, answer: 'Friend Service error: $error', provider: '-'));
      });
      _scrollToBottom();
    } finally {
  ...
  Future<List<String>> _nativeOpenFiles() async {
    if (!Platform.isWindows) {
      if (mounted) setState(() => _toolStatus = 'ตัวเลือกไฟล์ V3 รองรับ Windows desktop ใน build นี้');
      return const <String>[];
    }
    const script = r'''
...
''';
    try {
      final result = await Process.run(
        'powershell.exe',
        const <String>['-NoProfile', '-STA', '-Command', script],
      );
      if (result.exitCode != 0) throw ProcessException('powershell.exe', const <String>[], result.stderr.toString(), result.exitCode);
      ...
      if (mounted) setState(() => _toolStatus = 'เปิดตัวเลือกไฟล์ไม่สำเร็จ: $error');
      return const <String>[];
    }
  }
  ...
  Future<String?> _nativePickFolder() async {
    if (!Platform.isWindows) {
      if (mounted) setState(() => _toolStatus = 'ตัวเลือกคลัง V3 รองรับ Windows desktop ใน build นี้');
      return null;
    }
    const script = r'''
Add-Type -AssemblyName System.Windows.Forms
...
''';
    try {
      ...
      if (mounted) setState(() => _toolStatus = 'เปิดตัวเลือกคลังไม่สำเร็จ: $error');
      return null;
    }
  }
  ...
  Future<void> _openExternal(String url) async {
    ...
    try {
      await Process.start('rundll32.exe', <String>['url.dll,FileProtocolHandler', url]);
    } catch (error) {
      if (mounted) setState(() => _toolStatus = 'เปิดลิงก์ไม่สำเร็จ: $error');
    }
  }

  static String _leafName(String path) {
    final normalized = path.replaceAll('\\', '/');
    final segments = normalized.split('/').where((item) => item.isNotEmpty).toList();
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        final width = (constraints.maxWidth - 40).clamp(560.0, 1040.0).toDouble();
        return Center(
          child: SizedBox(
            width: width,
            child: Padding(
                        ...
                        children: _attachments
                            .map(
                              (attachment) => InputChip(
                                ...
                                label: Text(attachment.label, overflow: TextOverflow.ellipsis),
              ...
              Expanded(
                child: Column(
                  ...
                  children: <Widget>[
                    SelectableText(
                      turn.answer,
                      key: const Key('friend-v3-answer'),
...
class _Composer extends StatelessWidget {
  ...
  final bool busy;
  final _ChatMode mode;
  final Future<void> Function(_ChatMode) onModeSelected;
            ...
            Row(
              ...
              children: <Widget>[
                PopupMenuButton<_ChatMode>(
                  key: const Key('friend-v3-tools-menu'),
                  tooltip: 'เพิ่มเครื่องมือ',
                  onSelected: (value) => onModeSelected(value),
                ...
                IconButton(
                  tooltip: 'Voice input จะเชื่อมในขั้นถัดไป',
                  onPressed: null,
                  icon: const Icon(Icons.mic_none),
                ),
                FilledButton(
                  key: const Key('friend-v3-send'),
...
class _Capabilities extends StatelessWidget {
  const _Capabilities({required this.api, this.startup});
...
class _Memory extends StatelessWidget {
  const _Memory({required this.api});
...
class _Provider extends StatefulWidget {
  const _Provider({required this.api});
...
class _ProviderState extends State<_Provider> {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      padding: const EdgeInsets.all(28),
      children: <Widget>[
        Text('OpenAI-compatible Provider', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w700)),
        const SizedBox(height: 8),
        Text('Credential: ${credentialPresent ? 'stored securely' : 'not configured'} • backend: ${_status?['secret_backend'] ?? '-'}'),
        const SizedBox(height: 20),
        TextField(key: const Key('provider-v3-base-url'), controller: _baseUrl, decoration: const InputDecoration(labelText: 'Base URL')),
        const SizedBox(height: 12),
        TextField(key: const Key('provider-v3-model'), controller: _model, decoration: const InputDecoration(labelText: 'Model')),
        const SizedBox(height: 12),
        TextField(
          key: const Key('provider-v3-api-key'),
          controller: _apiKey,
          obscureText: true,
          decoration: const InputDecoration(labelText: 'API key (leave blank to keep existing)'),
        ),
        const SizedBox(height: 16),
        FilledButton.icon(
          key: const Key('provider-v3-save-test'),
        ...
        if (_message.isNotEmpty)
          Padding(
            ...
            child: SelectableText(_message, key: const Key('provider-v3-message')),
          ),
      ],
    );
  }
