...
final class HttpOwnerFriendApi implements OwnerFriendApi {
  ...
  Future<Map<String, dynamic>> _request(
    ...
    try {
      ...
      if (decoded is! Map) {
        throw const FormatException('Owner Friend response must be a JSON object');
      }
      final result = Map<String, dynamic>.from(decoded);
      if (response.statusCode != HttpStatus.ok) {
        throw HttpException('Owner Friend returned HTTP ${response.statusCode}: ${result['error']}', uri: uri);
      }
      return result;
    } finally {
