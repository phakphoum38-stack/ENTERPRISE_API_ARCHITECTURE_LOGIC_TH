...
class BoundedWorkerPool(Generic[T, R]):
    ...
    ``on_lease_failure`` receives renewal errors so the existing ownership
    layer can revoke/recover ownership; the pool does not create a second
    ownership model.
    """
    ...
    def __init__(
        ...
        *,
        ...
        on_lease_failure: Optional[Callable[[WorkerTask[T], BaseException], None]] = None,
        ...
        self._worker = worker
        self._queue: queue.Queue[WorkerTask[T]] = queue.Queue(maxsize=max_queue)
        self._max_workers = max_workers
        self._lease_renew = lease_renew
        self._on_failure = on_failure
        self._on_lease_failure = on_lease_failure
    ...
    def shutdown(self, *, wait: bool = True, timeout: Optional[float] = None) -> None:
        """Stop accepting work and let queued/running work drain.

        Workers poll the bounded queue with a short timeout and exit once
        shutdown has been requested and the queue is empty. This avoids
        inserting stop sentinels into a full bounded queue and therefore
        avoids an unbounded shutdown block.
        """
        with self._lock:
    ...
    def _renew_loop(self, task: WorkerTask[T], stop: threading.Event) -> None:
        while not stop.wait(self._renew_interval):
            try:
                ...
                if self._on_lease_failure is not None:
                    self._on_lease_failure(task, exc)
                stop.set()
                return
