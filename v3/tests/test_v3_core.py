...
class V3CleanCoreTests(unittest.TestCase):
    ...
    def test_adaptive_scale_profiles(self) -> None:
        cases = (
            ...
            (Workload(estimated_leaf_tasks=10000000001), ScaleTier.TIER_20_20, 20**20),
        )
    ...
    def test_max_profile_uses_backpressure_when_demand_exceeds_capacity(self) -> None:
        decision = self.master.decide(
            Workload(estimated_leaf_tasks=20**20 + 1)
        )
        self.assertEqual(decision.profile.tier, ScaleTier.TIER_20_20)
        self.assertEqual(decision.profile.capacity, 20**20)
    ...
    def test_master_contract_is_stable(self) -> None:
        ...
        self.assertEqual(payload["system_maximum_scale"], "20^20")
        self.assertEqual(payload["system_maximum_logical_capacity"], 20**20)
