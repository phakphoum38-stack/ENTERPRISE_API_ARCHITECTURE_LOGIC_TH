...
`Failure → Analysis → Repair Plan → repair.diff → Validate → Apply on task branch → Verify → Evidence → Continue`

## Boundaries

- Diff generation is proposal-only.
...
- Evidence should record the diff SHA-256, touched files, validation result, application result, and verification run.
