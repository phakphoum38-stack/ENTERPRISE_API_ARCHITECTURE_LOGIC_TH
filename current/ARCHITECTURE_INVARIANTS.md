...
- **INV-021 — One canonical registry per domain:** Governance metadata may reference canonical registries, but must not create competing registries or authorities.
- **INV-022 — No hard-coded surface cardinality:** Cross-platform surface validation derives destination identity and cardinality from the canonical navigation/product-surface sources.
- **INV-023 — Change impact is explicit:** Governed changes identify affected components, contracts, risk, blast radius, and required gates before release.
- **INV-024 — Evidence coverage is attributable:** Component completeness requires implementation, test, contract, evidence, provenance, and applicable gate coverage; file existence alone is insufficient.
