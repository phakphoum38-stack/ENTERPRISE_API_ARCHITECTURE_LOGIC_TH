...
class OwnerPackageTests(unittest.TestCase):
    def test_repository_owner_source_is_standalone_and_exact_allowlist(self) -> None:
        report = validate_source_tree(_REPO_ROOT)
        self.assertTrue(report["validated"])
        self.assertFalse(report["unexpected_dependencies_found"])
        self.assertEqual(report["files"], list(OWNER_PACKAGE_FILES))
        self.assertTrue(report["standalone"])
    ...
    def test_owner_package_rejects_dependency_outside_standalone_allowlist(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            source = root / OWNER_PACKAGE_FILES[0]
            source.parent.mkdir(parents=True, exist_ok=True)
            source.write_text("import subprocess\n", encoding="utf-8")
            with self.assertRaisesRegex(OwnerPackageError, "dependencies outside the standalone allowlist"):
