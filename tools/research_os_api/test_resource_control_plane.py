...
class ResourceControlPlaneTests(unittest.TestCase):
    ...
    def test_execute_commits_measured_execution_and_evidence(self):
        result = self.plane.execute(request_id="req-1", principal_id="user-1", objective="research", usage=Usage(requests=1), estimated_cost=Decimal("2.00"), currency="USD", scopes=frozenset({"agent:run"}), available_providers=("local",), executor=lambda _route: MeasuredExecution({"provider": "local", "model": "research", "text": "ok"}, Usage(requests=1), Decimal("1.25"), "USD"))
    ...
    def test_idempotency_replay_fails_closed_without_second_execution(self):
        ...
        first = self.plane.execute(request_id="first", executor=lambda _route: calls.append("first") or MeasuredExecution({"provider": "local", "model": "research", "text": "ok"}, Usage(requests=1), Decimal("0.50"), "USD"), **common)
        replay = self.plane.execute(request_id="retry", executor=lambda _route: calls.append("retry") or MeasuredExecution({"text": "duplicate"}, Usage(requests=1), Decimal("0.50"), "USD"), **common)
    ...
    def test_invalid_measured_currency_releases_reservation(self):
        with self.assertRaises(ValueError):
            self.plane.execute(request_id="bad-currency", principal_id="user-1", objective="research", usage=Usage(requests=1), estimated_cost=Decimal("1.00"), currency="USD", scopes=frozenset({"agent:run"}), available_providers=("local",), executor=lambda _route: MeasuredExecution({"text": "bad"}, Usage(requests=1), Decimal("1.00"), "EUR"))
