...
def scan_workflows() -> tuple[dict[str, Any], list[dict[str, str]], list[dict[str, str]], list[str]]:
    ...
    for path in sorted(WORKFLOWS.glob("*.y*ml")):
        ...
        target_sha = {
            ...
            "checkout_ref": bool(re.search(r"checkout@[^\n]+[\s\S]{0,400}ref:\s*\$?\{?\$?\{?env\.TARGET_SHA|ref:\s*\$\{\{\s*inputs\.target_sha", text)),
...
def render_txt(records: dict[str, Any], edges: list[dict[str, str]], artifact_edges: list[dict[str, str]], issues: list[dict[str, str]], ref: str) -> str:
    ...
    lines = [
        "RESEARCH OS WORKFLOW INTELLIGENCE AUDIT",
        "=" * 72,
        f"Ref: {ref}",
        f"Workflows scanned: {len(records)}",
    ...
    if issues:
        ...
        lines.append("NONE")
    lines += ["", "CANONICAL RELEASE SPINE", "-" * 72, " -> ".join(SPINE)]
    lines += ["", f"FINAL RESULT: {'FAIL' if fail else 'PASS WITH WARNINGS' if warn else 'PASS'}"]
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    md = "# Workflow Build Order Audit\n\n```text\n" + txt + "```\n"
    (REPORTS / "WORKFLOW_BUILD_ORDER.md").write_text(md, encoding="utf-8")
    print(txt, end="")
