#!/usr/bin/env python3
"""Extract and classify the reverted Control Center #521 payload without mutating source.
...
DEFAULT_PATHS = [
    "apps/research_os_flutter/lib/src/features/control_center",
    "apps/research_os_flutter/lib/src/app_shell.dart",
    "apps/research_os_flutter/test/features/control_center",
]
...
def changed_paths(base: str, payload: str) -> list[str]:
    out = run("diff", "--name-status", base, payload)
...
def classify(path: str, text: str) -> dict[str, Any]:
    ...
    real_api = any(x in lower for x in ("research_os_api_client", "gethealth()", "getbraincapacity()", "getproviders()", "getagents()"))
    projection = any(x in lower for x in ("unknown is preserved", "descriptive_only", "canonical closure projection", "runtime observation"))
    authority = any(x in lower for x in ("authorize", "approval", "release", "merge")) and "external" in lower
...
def main() -> int:
    ...
    parser.add_argument("--payload-ref", default="7d6db9ccca7c3565288411a4cf01f5b5219bdb07")
    parser.add_argument("--base-ref", default="37b2276f2ed8a31d58ff92a266e46f1c9a7098a4")
    parser.add_argument("--out", default="reports/forensic/control-center-payload/CONTROL_CENTER_521_FORENSIC.json")
