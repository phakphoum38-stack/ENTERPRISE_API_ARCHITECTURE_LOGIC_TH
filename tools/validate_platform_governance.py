...
def validate() -> list[str]:
    ...
    canonical_release = [
        d for d in domains
        if d.get("domain") == "release" and d.get("canonical") == "current/RESEARCH_OS_UNIFIED_FINAL_GATE.yml"
    ]
    if len(canonical_release) != 1:
    ...
    if marker not in navigation:
        ...
        for entry in entries:
            ...
            if labels_match:
                nav_labels.append(labels_match.group(2))
        ...
        if nav_labels and len(nav_labels) != len(set(nav_labels)):
            failures.append("navigation_duplicate_label")

    invariants = (ROOT / "current/ARCHITECTURE_INVARIANTS.md").read_text(encoding="utf-8")
    for needle in ("INV-014", "INV-018", "INV-020", "Enforcement Principle"):
