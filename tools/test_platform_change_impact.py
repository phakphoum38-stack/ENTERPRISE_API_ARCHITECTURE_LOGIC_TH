...
class PlatformChangeImpactTests(unittest.TestCase):
    def test_navigation_change_expands_to_surface_and_gates(self):
        result = subprocess.run([sys.executable, "tools/platform_change_impact.py", "apps/research_os_flutter/lib/src/ui/enterprise_navigation.dart"], cwd=ROOT, text=True, capture_output=True, check=True)
    ...
    def test_unknown_path_has_no_invented_impact(self):
        result = subprocess.run([sys.executable, "tools/platform_change_impact.py", "docs/unrelated.txt"], cwd=ROOT, text=True, capture_output=True, check=True)
        payload = json.loads(result.stdout)
