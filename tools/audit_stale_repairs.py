...
def main() -> int:
    ...
    result = {
        "version": 1,
        "policy": {"stale_after_days": args.days, "mutate_automatically": False},
        "candidates": candidates,
        "stale": [x for x in candidates if x["stale"]],
        "duplicate_fingerprint_prs": duplicates,
        "status": "CLEAN" if not any(x["stale"] for x in candidates) and not duplicates else "REVIEW_REQUIRED",
    }
