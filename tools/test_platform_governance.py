...
class PlatformGovernanceTests(unittest.TestCase):
    def test_contract_is_active_and_fail_closed(self):
        contract = json.loads((ROOT / "current/RESEARCH_OS_PLATFORM_GOVERNANCE_CONTRACT.json").read_text(encoding="utf-8"))
    ...
    def test_validator_passes_against_canonical_sources(self):
        result = subprocess.run([sys.executable, "tools/validate_platform_governance.py"], cwd=ROOT, text=True, capture_output=True)
