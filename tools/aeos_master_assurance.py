...
def main() -> int:
    ...
    discovery_failures = [item for item in discovery.get("dimensions", []) if item.get("status") in FAILURE_STATUSES]
    ...
    manifest = {
        ...
        "failed": len(failed),
        "decision": decision,
        "decision_reasons": ["control_failure" if failed else "all_controls_pass", "fix_regression" if fix_verification["status"] == "FAIL" else "fix_verification_not_failed", "umap_discovery_failure" if discovery_failures else "umap_discovery_pass"],
        "change_scope": git_scope(git_base_sha(), source_sha),
        "fix_verification": fix_verification,
        "umap_discovery": discovery,
