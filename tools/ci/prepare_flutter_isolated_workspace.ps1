...
function Get-LocalPathDependencies {
    ...
    foreach ($line in $lines) {
        ...
        if ($currentDependency -ne $null -and $indent -eq $currentDependencyIndent -and $trimmed -match '^([A-Za-z0-9_.-]+):\s*
    }
...
Write-Host "RESEARCH_OS_ISOLATED_APP=$appTarget"
        ...
        if ($currentDependency -ne $null -and $indent -lt $currentDependencyIndent) {
            $currentDependency = $null
        }
    }
