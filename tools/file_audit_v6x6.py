...
BRANCHING_FACTOR = 6
DEPTH = 6
MAX_LEAF_CAPACITY = BRANCHING_FACTOR ** DEPTH  # 46,656
...
def audit_file(path: Path, root: Path) -> FileResult:
    ...
    try:
        ...
        return FileResult(
            rel,
            (Finding(rel, "error", "unreadable", f"Cannot stat file: {exc}"),),
        )
        ...
        if pattern.search(text):
            findings.append(
                Finding(
                    ...
                    "error",
                    code,
                    "Credential-like material found; verify and remove or rotate if real.",
                )
            )
    ...
    if suffix == ".json":
        try:
            ...
            findings.append(
                Finding(rel, "error", "invalid-json", f"JSON parse error: {exc.msg} at line {exc.lineno}.")
            )
    elif suffix == ".py":
        try:
