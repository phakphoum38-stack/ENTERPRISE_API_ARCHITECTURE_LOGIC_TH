...
def validate(contract,registry):
    ...
    if d.get("mode")!="bounded_hierarchical":f.append("discovery_mode")
    for key in ("same_level_first","virtual_space_required","root_required_before_create","negative_cache","positive_cache","cross_space_duplicate_detection","capability_duplicate_detection","stale_candidate_detection","emergency_stop","proof_required"):
        if d.get(key) is not True:f.append(f"discovery:{key}")
    if d.get("ambiguous_policy")!="HOLD":f.append("discovery:ambiguous_policy")
    if d.get("root_unresolved_policy")!="STOP":f.append("discovery:root_unresolved_policy")
    if d.get("timeout_policy")!="RETURN_PROOF":f.append("discovery:timeout_policy")
    if not 1 <= int(d.get("max_time_ms",0)) <= 300000:f.append("discovery:max_time_ms")
    if not 1 <= int(d.get("max_depth",0)) <= 256:f.append("discovery:max_depth")
