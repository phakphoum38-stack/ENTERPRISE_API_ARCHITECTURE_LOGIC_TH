...
class PlatformCoreCompletionTests(unittest.TestCase):
    ...
    def test_project_isolation(self) -> None:
        ...
        self.assertEqual(
            (),
            project_scale_readiness.validate_isolation(
                project_a=contexts[0],
                project_b=contexts[1],
            ),
        )
    ...
    def test_duplicate_project_id_is_rejected(self) -> None:
        ...
        self.assertIn(
            ...
            project_scale_readiness.validate_project_contexts(tuple(contexts)),
        )

    def test_incomplete_idempotency_identity_is_rejected(self) -> None:
        with self.assertRaises(ValueError):
            project_scale_readiness.build_idempotency_key(
    ...
    def test_cross_project_idempotency_is_distinct(self) -> None:
        key_a = project_scale_readiness.build_idempotency_key(
            project_id="project-001",
            event_id="event-1",
            source_sha="a" * 40,
            action="execute",
        )
        key_b = project_scale_readiness.build_idempotency_key(
