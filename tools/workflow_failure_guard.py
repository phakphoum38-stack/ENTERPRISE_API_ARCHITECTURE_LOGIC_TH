...
def main() -> int:
    ...
    result = {"version": 1, "fingerprint": fingerprint, "duplicate": duplicate, "recent_investigation_issues": recent, "rate_limit": {"max": args.limit, "window_minutes": args.window_minutes}, "allowed": allowed, "reason": "DUPLICATE" if duplicate else ("RATE_LIMIT" if recent >= args.limit else "ALLOW"), "issue_title": title}
