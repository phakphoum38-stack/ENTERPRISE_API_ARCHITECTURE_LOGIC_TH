#!/usr/bin/env python3
"""Generate a target-bound provenance ledger and evidence root in CI.

The generated files are runtime evidence, not source-of-truth fixtures. The
commit under test is taken from git, its immediate parent is recorded as the
source generation, and all derived hashes are computed locally.
"""
...
def main() -> int:
    ...
    change = {
        ...
        "evidence": {
            "source_commit": source,
            "target_commit": target,
            "evidence_ids": [attestation_id],
            "attestation_id": attestation_id,
        },
        "previous_entry_hash": attestation["entry_hash"],
    }
    change["entry_hash"] = digest(change)

    ledger = {
        "contract_version": "1.1.0",
        "entries": [attestation, change],
    }
