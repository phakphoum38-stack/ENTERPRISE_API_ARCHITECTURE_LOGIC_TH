...
_SESSION_STATE: dict[str, tuple[str, str | None]] = {}
...
def inspect()->dict[str,Any]:
 ...
 for provider,c in PROVIDERS.items():
  ...
  if provider in _SESSION_STATE:
   state,route=_SESSION_STATE[provider]
  else:
...
def connect(provider:str)->dict[str,Any]:
 ...
 _SESSION_STATE[provider]=(state,route)
...
def disconnect(provider:str)->dict[str,Any]:
 provider=provider.strip().lower()
 if provider not in PROVIDERS:raise AIProviderConnectionError(f"unsupported provider: {provider}")
 _SESSION_STATE[provider]=("NOT_CONNECTED",None)
 evidence=_evidence(provider,"DISCONNECT","NOT_CONNECTED",detail="connection_state_cleared_credentials_retained");_append_evidence(evidence)
