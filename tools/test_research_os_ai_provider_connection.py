...
from tools.research_os_ai_provider_connection import inspect, _configured
...
class AIProviderConnectionTests(unittest.TestCase):
 ...
 def test_gemini_connected_when_secret_and_model_exist(self):
  with patch.dict(os.environ,{"RESEARCH_OS_GEMINI_API_KEY":"secret","RESEARCH_OS_GEMINI_MODEL":"gemini-test"},clear=True):
   item=_configured("gemini")
  self.assertEqual(item.state,"CONNECTED")
  self.assertFalse(item.secret_exposed)

 def test_inspect_never_contains_secret(self):
  with patch.dict(os.environ,{"RESEARCH_OS_OPENAI_API_KEY":"TOP-SECRET","RESEARCH_OS_OPENAI_MODEL":"gpt-test"},clear=True):
   report=inspect()
