...
import 'package/src/api/research_os_api_client.dart';
import 'package/src/app_shell.dart';
...
Future<void> openDesktopDestination(
WidgetTester tester,
int index,
) async {
...
testWidgets(
'home dashboard shows Research OS status',
(tester) async {
...
setDesktopTestSize(tester);
  ...
  // Start _send() and allow the UI to rebuild.
  await tester.pump();

  // Allow answerWithMemory() and persistence to complete.
  await tester.pump(
    const Duration(milliseconds: 500),
  );

  // Flush any pending microtasks/rebuilds.
  await tester.pump();

  expect(
    find.text(
      'คำตอบจาก Gemini ที่ใช้ความรู้ในห้องสมุด',
    ),
    findsOneWidget,
  );

  expect(
    find.text('Memory 1 รายการ'),
    findsOneWidget,
  );
...
testWidgets(
'library shows knowledge artifacts',
(tester) async {
setDesktopTestSize(tester);
  ...
  expect(
    find.text('Conversation to Knowledge'),
    findsOneWidget,
  );
...
setDesktopTestSize(tester);
  ...
  expect(
    find.byKey(
      const Key('knowledge-graph-heading'),
    ),
    findsOneWidget,
  );
  ...
  expect(
    find.text('Knowledge Graph'),
    findsWidgets,
  );

  expect(
    find.text('RES-A -> RES-B'),
    findsOneWidget,
  );

  expect(
    find.textContaining('supports'),
    findsWidgets,
  );
...
testWidgets(
'GitHub dashboard shows workflows and commits',
(tester) async {
setDesktopTestSize(tester);
  ...
  expect(
    find.text('Research OS Flutter'),
    findsOneWidget,
  );

  expect(
    find.text('Add GitHub dashboard'),
    findsOneWidget,
  );
...
testWidgets(
'settings shows provider and changes theme mode',
(tester) async {
setDesktopTestSize(tester);
