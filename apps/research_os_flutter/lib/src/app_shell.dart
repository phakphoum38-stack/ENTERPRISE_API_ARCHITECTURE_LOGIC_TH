...
import 'features/api_live/api_live_demo_page.dart';
...
class ResearchOSAppShell extends StatefulWidget {
  const ResearchOSAppShell({required this.apiClient, this.themeMode = ThemeMode.system, this.onThemeModeChanged, this.onApiBaseUrlChanged, super.key});
...
class _ResearchOSAppShellState extends State<ResearchOSAppShell> {
  ...
  List<Widget> get _pages => <Widget>[
        ...
        SettingsPage(apiClient: widget.apiClient, themeMode: widget.themeMode, onThemeModeChanged: widget.onThemeModeChanged ?? (ThemeMode value) {}, onApiBaseUrlChanged: widget.onApiBaseUrlChanged),
        DeveloperAccessPage(),
        BrainSkillsPage(apiClient: widget.apiClient),
        GoogleLoginPage(apiClient: widget.apiClient),
        ApiLiveDemoPage(apiClient: widget.apiClient),
      ];

  void _select(int index) => setState(() => _selectedIndex = index);
  void _toggleSidebar() => setState(() => _sidebarExpanded = !_sidebarExpanded);
  void _selectFromSidebar(int index) => setState(() { _selectedIndex = index; _sidebarExpanded = false; });

  @override
  Widget build(BuildContext context) {
    final current = researchNavigationItems.firstWhere((item) => item.index == _selectedIndex);
    return LayoutBuilder(builder: (context, constraints) {
      if (constraints.maxWidth >= 920) {
        return Scaffold(body: SafeArea(child: Row(children: <Widget>[
          ResearchOSSidebarV2(expanded: _sidebarExpanded, selectedIndex: _selectedIndex, onToggle: _toggleSidebar, onSelected: _selectFromSidebar),
          Expanded(child: Column(key: const Key('desktop-content-pane'), children: <Widget>[
      ...
      return Scaffold(
        drawer: ResearchMobileDrawer(selectedIndex: _selectedIndex, onSelected: (index) { Navigator.of(context).pop(); _select(index); }),
        appBar: AppBar(titleSpacing: 0, title: Row(children: <Widget>[const ResearchBrandMark(compact: true), const SizedBox(width: 10), Expanded(child: Text(current.label, overflow: TextOverflow.ellipsis))]), actions: <Widget>[IconButton(tooltip: 'System Monitor', onPressed: () => _select(8), icon: const Icon(Icons.monitor_heart_outlined))]),
