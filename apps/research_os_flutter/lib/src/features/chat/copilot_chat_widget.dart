...
class _CopilotChatWidgetState extends State<CopilotChatWidget> {
  ...
  Future<void> _send() async {
    ...
    try {
      ...
      final context = (result['context'] as Map?)?.cast<String, dynamic>() ?? const <String, dynamic>{};
      ...
      final reply = (result['reply'] ?? result['text'] ?? 'Copilot returned no reply.').toString();
      if (!mounted) return;
      setState(() {
        _contextSummary = '$repository • $files files • $memoryCount memory hits';
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            Text('Copilot Chat', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w800)),
            const SizedBox(height: 6),
            Text('Enterprise API widget with trusted repository context', style: Theme.of(context).textTheme.bodySmall),
            ...
            Container(
              ...
              child: Text(_contextSummary ?? 'พร้อมส่งบริบทจาก repository และ trusted user scope'),
            ),
            const SizedBox(height: 12),
            Expanded(
              child: _turns.isEmpty
                  ? const Center(child: Text('ยังไม่มีข้อความ เริ่มถาม Copilot ได้เลย'))
                  : ListView.builder(
              ...
              children: <Widget>[
                Expanded(
                  child: TextField(
                    ...
                    minLines: 1,
                    maxLines: 4,
                    decoration: const InputDecoration(labelText: 'Ask Copilot', hintText: 'เช่น ช่วยสรุป architecture และความเสี่ยง'),
                    onSubmitted: (_) => _send(),
                  ),
                ),
